...
export const Row: Printable<Props> = ({
  left,
  center,
  right,
  gap,
...
Row.print = (elem, context) => {
  ...
  const centerElem =
    typeof center === "string" ? <Text>{center}</Text> : center;
  ...
  const centerString =
    centerElem !== undefined
      ? reactNodeToString(centerElem.props.children)
      : undefined;
  ...
  const leftLineWidth =
    centerElem !== undefined ? leftLength : width - gap - rightLength;
  const leftLines = wrapText(leftString, {
    size: leftSize,
    width: leftLineWidth,
  });
  const centerLineWidth =
    width - Math.max(gap * 2, 1) - leftLength - rightLength;
  const centerLines =
...
function lineText(
  textElem: ReactElement<ComponentProps<typeof Text>>,
  text: string
) {
